...
class _SelectSubjectsScreenState extends State<SelectSubjectsScreen> {
  ...
  bool hasAllSubjectElected(
      {required ClassElectiveSubjectsFetchSuccess classState,}) {
    //logic to check if all group's elective subjects are properly selected
    bool subjectElected = true;
    for (final key in selectedElectiveSubjects.keys) {
      //if a group does not exist in selectedElectiveSubjects the selection is not done
      if (selectedElectiveSubjects[key]!.isEmpty) {
        subjectElected = false;
        break;
      } else if (selectedElectiveSubjects[key]!.length <
          classState.electiveSubjectGroups
              .where((element) => element.id == key)
              .first
              .totalSelectableSubjects) {
        //if a group has less number of subjects than the total selectable subjects in that group
        subjectElected = false;
        break;
      }
    }
  ...
  Widget _buildAppBar() {
    final currentClass = context
        .read<AuthCubit>()
        .getStudentDetails()
        .classSectionNameWithSemester(context: context);
    return Align(
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          ...electiveSubjectGroup.subjects.map(
      ...
      child: BlocConsumer<SelectElectiveSubjectsCubit,
        ...
        listener: (context, state) {
          if (state is SelectElectiveSubjectsFailure) {
            ...
            for (final electiveSubjectGroup in electiveSubjectGroups) {
        ...
        builder: (context, state) {
          return CustomRoundedButton(
            onTap: () {
              ...
              if (hasAllSubjectElected(classState: classState)) {
                context
            ...
            BlocConsumer<ClassElectiveSubjectsCubit,
              ...
              listener: (context, state) {
                if (state is ClassElectiveSubjectsFetchSuccess) {
                  //
                  for (final electiveSubjectGroup
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Center(child: _buildSubmitButton(classState: state)),
                          const SizedBox(
                            height: 20,
                          ),
                        ],
