...
class Leave {
  ...
  String formattedDateRange({required BuildContext context}) {
    return "${UiUtils.formatDate(DateTime.parse(fromDate))}${fromDate != toDate ? ' ${UiUtils.getTranslatedLabel(context, toKey)} ${UiUtils.formatDate(DateTime.parse(toDate))}' : ''}";
  }
  ...
  factory Leave.fromJson(Map<String, dynamic> json) {
    return Leave(
      ...
      leaveDetail: List<LeaveDetail>.from(json['leave_detail']?.map((x) => LeaveDetail.fromJson(x)) ?? []),
      file: List<StudyMaterial>.from(json['file']?.map((x) => StudyMaterial.fromJson(x)) ?? []),
    );
  }
}
...
Month getCurrentMonth() {
  return getAllMonths()[DateTime.now().month - 1];
}
