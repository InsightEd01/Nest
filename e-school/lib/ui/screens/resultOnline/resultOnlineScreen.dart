    ...
    return CupertinoPageRoute(
      builder: (_) => BlocProvider(
        ...
        child: ResultOnlineScreen(
          ...
          totalMarks: arguments["totalMarks"],
...
class ResultOnlineScreenState extends State<ResultOnlineScreen> {
            ...
            children: [
              ...
              Column(
                children: [
                  ...
                  10.sizedBoxHeight,
                  BlocBuilder<ResultOnlineCubit, ResultOnlineState>(
                    builder: (context, state) {
                      String obtainedMarks = widget.obtainedMarks ?? "";
                      String totalMarks = widget.totalMarks ?? "";
                      if (state is ResultOnlineFetchSuccess) {
                        obtainedMarks = state.result.totalObtainedMarks ?? "";
                        totalMarks = state.result.totalMarks ?? "";
                      }
                      return CustomRoundedButton(
                        ...
                        buttonTitle:
                            "${UiUtils.getTranslatedLabel(context, obtainedMarksKey)} :  $obtainedMarks / $totalMarks",
                        textSize: 14,
                        showBorder: true,
                      );
                    },
    ...
    return Row(
      ...
      children: [
        Text(
          ...
          style: setTextStyle(
            txtColor: UiUtils.getColorScheme(context).onSurface,
            txtSize: 16.0,
  ...
  Container buildShimmerLoadingResultDetailsContainer() {
    return Container(
      ...
      padding: EdgeInsets.only(
        bottom: 15,
        top: MediaQuery.of(context).size.height * 0.285 + 15,
      ),
      child: LayoutBuilder(
        builder: (context, boxConstraints) {
      ...
      body: Stack(
        children: [
          BlocBuilder<ResultOnlineCubit, ResultOnlineState>(
            builder: (context, state) {
              if (state is ResultOnlineFetchSuccess) {
                return buildResultSummary(state.result);
              }
